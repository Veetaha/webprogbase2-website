'use strict';
Object.defineProperty(exports, '__esModule', { value: true });
const path = require('path');
const fs = require('fs');
const md5 = require('md5');
const v1_1 = require('./public-api/v1/index');
// import * as dotenv from 'dotenv';
// dotenv.load();
exports.ProjectRootDir = path.normalize(path.join(__dirname, '../../'));
exports.Port = process.env.PORT || 8080;
exports.MongoDbUri = process.env.DATABASE_URL || 'mongodb://localhost:27017/veedb';
exports.PasswordSalt = process.env.PASSWORD_SALT || 'dev-password-salt';
exports.JwtEncodingAlgorithm = process.env.JWT_ENCODING_ALG || 'RS256';
// expressed in seconds or a string describing a time span zeit/ms. Eg: 60, "2 days", "10h", "7d"
exports.JwtExpirationTime = process.env.JWT_EXP_TIME || '2 days';
exports.DefaultRegisteredUserRole = process.env.DEFAULT_REG_USER_ROLE || v1_1.UserRole.Student;
exports.DefaultUserAvatarUrl = process.env.DEFAULT_USER_AVA_URL || '/assets/images/default-user-avatar.png';
exports.GqlSchema = fs.readFileSync(relativeToRoot('backend/public-api/v1/schema.gql')).toString();
exports.FrontendDistDir = relativeToRoot('frontend/dist/lab5');
exports.DocsDir = relativeToRoot('backend/docs');
exports.FrontendIndexPath = relativeToRoot('frontend/dist/lab5/index.html');
function relativeToRoot(dest) {
    return path.join(exports.ProjectRootDir, dest);
}
function encodePassword(password) {
    return md5(`${ password }${ exports.PasswordSalt }`);
}
exports.encodePassword = encodePassword;
exports.RsaPublicKey = process.env.RSA_PUBLIC_KEY || `-----BEGIN RSA PUBLIC KEY-----

tGCKIQ0/btqK+jFunT2NBC8RItanDZpptQIDAQAB
-----END RSA PUBLIC KEY-----`;
exports.RsaPrivateKey = process.env.RSA_PRIVATE_KEY || `-----BEGIN RSA PRIVATE KEY-----

-----END RSA PRIVATE KEY-----`;